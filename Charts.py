...
class Charts:
    ...
    def draw_trade_result(self, trade_df):
        ...
        line.add_yaxis('最高价', y_axis=trade_df['最高价'].tolist(), symbol_size=1,linestyle_opts=opts.LineStyleOpts(width=2))#,yaxis_index=1
        line.add_yaxis('最低价', y_axis=trade_df['最低价'].tolist(), symbol_size=1,linestyle_opts=opts.LineStyleOpts(width=2))
        line.add_yaxis('日前价格(压缩平均)', y_axis=trade_df['日前价格(压缩平均)'].tolist(), symbol_size=1,linestyle_opts=opts.LineStyleOpts(width=2,type_='dashed'))
        line.add_yaxis('成交均价',
        ...
        bar.add_yaxis('交易电量(日均)+', y_axis=trade_df_rep['交易电量(日均)'][trade_df_rep['交易电量(日均)'] >= 0].tolist(), z=0, itemstyle_opts=opts.ItemStyleOpts(color='green'))
        bar.add_yaxis('交易电量(日均)-', y_axis=trade_df_rep['交易电量(日均)'][trade_df_rep['交易电量(日均)'] <= 0].tolist(), z=0,itemstyle_opts=opts.ItemStyleOpts(color='red'))
        ...
        bar.set_global_opts(legend_opts=opts.LegendOpts(type_="scroll", pos_right="0",pos_top='5%',orient="vertical"),
            title_opts=opts.TitleOpts(pos_left='center',#text_align='center',
                                      subtitle='总交易电量:' + str(df_trade['交易电量(日均)'].sum().round(3)) + 'MWh',subtitle_textstyle_opts=opts.TextStyleOpts(font_size=20),
                                      title='交易结果(交易日:'+ str(trade_df.iloc[0].交易日) + '---运行日:' + str(trade_df.iloc[0].运行日) + ')',title_textstyle_opts=opts.TextStyleOpts(font_size=23)),
                                      tooltip_opts=opts.TooltipOpts(trigger='axis'),
                            # toolbox_opts=opts.ToolboxOpts(),
        ...
        if mode == '运行日':
            ...
            bar.set_global_opts(legend_opts=opts.LegendOpts(type_="scroll", pos_right="0",pos_top='5%',orient="vertical"),
                                 title_opts=opts.TitleOpts(title_textstyle_opts=opts.TextStyleOpts(font_size=23),pos_left='center', title='盈亏结果(运行日:'+ str(profit_df.iloc[0].运行日)+')',
                                                           subtitle = '总盈亏:'+ str(profit_df['总盈亏(万元)'].sum().round(3)) + '万元    '
                                                                      +'偏差处理盈亏:' + str(profit_df['偏差处理/主动套利盈亏'][df_profit['盈亏类型'] == 0].sum().round(1)) + '元    '
                                                                      +'主动套利盈亏:' + str(profit_df['偏差处理/主动套利盈亏'][df_profit['盈亏类型'] == 1].sum().round(1)) + '元',
            ...
            bar.set_global_opts(legend_opts=opts.LegendOpts(type_="scroll", pos_right="0",pos_top='5%',orient="vertical"),
                                title_opts=opts.TitleOpts(title_textstyle_opts=opts.TextStyleOpts(font_size=23),pos_left='center', title='盈亏结果(交易日:'+ str(profit_df.iloc[0].交易日) + '---运行日:' + str(profit_df.iloc[0].运行日) + ')',subtitle = '总盈亏:'+ str(profit_df['总盈亏(万元)'].sum().round(3)) + '万元    '
                                                                      ...
                                                                      +'主动套利盈亏:' + str(profit_df['偏差处理/主动套利盈亏'][df_profit['盈亏类型'] == 1].sum().round(3)) + '元',
                                                           subtitle_textstyle_opts=opts.TextStyleOpts(font_size=20)),
                                # tooltip_opts=opts.TooltipOpts(trigger='axis'),
                                # toolbox_opts=opts.ToolboxOpts(),
                                xaxis_opts=opts.AxisOpts(),
    ...
    def draw_price_compare(self, df_price, is_compress, begin_date, end_date):
        ...
        if not is_compress:
            line1.add_yaxis('实时价格', df_price.groupby('运行日期')['实时价格'].mean().round(2).tolist(),markpoint_opts=opts.MarkPointOpts(data=[opts.MarkPointItem(type_='max', symbol='pin', symbol_size=80)]))
            line1.add_yaxis('日前价格', df_price.groupby('运行日期')['日前价格'].mean().round(2).tolist(),markpoint_opts=opts.MarkPointOpts(data=[opts.MarkPointItem(type_='max', symbol='pin', symbol_size=80)]))
            line2.add_yaxis('实时价格', df_price.groupby('时间')['实时价格'].mean().round(2).tolist(),markpoint_opts=opts.MarkPointOpts(data=[opts.MarkPointItem(type_='max', symbol='pin', symbol_size=80)]))
            line2.add_yaxis('日前价格', df_price.groupby('时间')['日前价格'].mean().round(2).tolist(),markpoint_opts=opts.MarkPointOpts(data=[opts.MarkPointItem(type_='max', symbol='pin', symbol_size=80)]))
        else:
            line1.add_yaxis('实时价格(压缩)', df_price.groupby('运行日期')['实时价格(压缩)'].mean().round(2).tolist(),markpoint_opts=opts.MarkPointOpts(data=[opts.MarkPointItem(type_='max', symbol='pin', symbol_size=80)]))
            line1.add_yaxis('日前价格(压缩)', df_price.groupby('运行日期')['日前价格(压缩)'].mean().round(2).tolist(),markpoint_opts=opts.MarkPointOpts(data=[opts.MarkPointItem(type_='max', symbol='pin', symbol_size=80)]))
            line2.add_yaxis('实时价格(压缩)', df_price.groupby('时间')['实时价格(压缩)'].mean().round(2).tolist(),markpoint_opts=opts.MarkPointOpts(data=[opts.MarkPointItem(type_='max', symbol='pin', symbol_size=80)]))
            line2.add_yaxis('日前价格(压缩)', df_price.groupby('时间')['日前价格(压缩)'].mean().round(2).tolist(),markpoint_opts=opts.MarkPointOpts(data=[opts.MarkPointItem(type_='max', symbol='pin', symbol_size=80)]))
    ...
    def draw_price_jjkj(self, df_compete, is_compress):
        ...
        bar.extend_axis(yaxis=opts.AxisOpts(min_=0, max_=100, position="right",axislabel_opts=opts.LabelOpts(formatter="{value}%"), interval=10,axisline_opts=opts.AxisLineOpts(is_show=True)))
        bar.extend_axis(yaxis=opts.AxisOpts(min_=0, max_=1500, position="right",axislabel_opts=opts.LabelOpts(formatter="{value}"), offset=80,axisline_opts=opts.AxisLineOpts(is_show=True)))
        bar.set_global_opts(
    ...
    def draw_jjkj_curve(self, df_compete, date_list):
        ...
        for date in date_list:
            scat1.add_xaxis(df_compete['竞价空间'][pd.to_datetime(df_compete['运行日期']) == pd.to_datetime(date)].unique().tolist())
            scat1.add_yaxis(date, y_axis=df_compete['日前价格'][pd.to_datetime(df_compete['运行日期']) == pd.to_datetime(date)].tolist(), symbol_size=10, label_opts=opts.LabelOpts(is_show=False))
        ...
        for date in date_list:
            scat2.add_xaxis(df_compete['竞价容量比%'][pd.to_datetime(df_compete['运行日期']) == pd.to_datetime(date)].unique().tolist())
            scat2.add_yaxis(date, y_axis=df_compete['日前价格'][pd.to_datetime(df_compete['运行日期']) == pd.to_datetime(date)].tolist(), symbol_size=10, label_opts=opts.LabelOpts(is_show=False))
        scat2.set_global_opts(
            legend_opts=opts.LegendOpts(type_="scroll", pos_left="right", orient="vertical"),
            title_opts=opts.TitleOpts(title='竞价容量比-日前价格', pos_left='30px'),
            yaxis_opts=opts.AxisOpts(type_='value', splitline_opts=opts.SplitLineOpts(is_show=True)),
            xaxis_opts=opts.AxisOpts(type_='value', splitline_opts=opts.SplitLineOpts(is_show=True), min_=0, max_=100, axislabel_opts=opts.LabelOpts(formatter="{value}%")),
            toolbox_opts=opts.ToolboxOpts(),
            tooltip_opts=opts.TooltipOpts(trigger_on='mousemove|click',axis_pointer_type='cross',formatter=JsCode(
        ...
        for date in date_list:
            data = list(zip(df_compete['竞价容量比%'][pd.to_datetime(df_compete['运行日期']) == pd.to_datetime(date)].tolist(), df_compete['竞价空间'][pd.to_datetime(df_compete['运行日期']) == pd.to_datetime(date)].tolist(),
...
if __name__ == '__main__':
    ...
    if agree == '市场动态':
        if scdt == '信息看板':
            ...
            trade_date = c21.selectbox('交易日', df_trade_result['交易日'].sort_values(ascending=False).unique().tolist(),index=0)
            run_date = c22.selectbox('运行日',df_trade_result['运行日'][df_trade_result['交易日'] == trade_date].unique().tolist(),index=0)
            ...
            if mode == '运行日':
                exec_date = c_32.selectbox('运行日期', df_trade_result['运行日'].sort_values(ascending=False).unique().tolist(),index=24)
        ...
        if scfx == '现货价格分析':
            ...
            with tab3:
                ...
                end_date = t3_cc43.selectbox('结束日期', df_info_price['运行日期'][pd.to_datetime(df_info_price['运行日期']) >= pd.to_datetime(begin_date)].sort_values(ascending=False).unique().tolist(), index=0, key='tab3_end_date')
                df_compete = df_info_price[(pd.to_datetime(df_info_price['运行日期']) >= pd.to_datetime(begin_date)) & (pd.to_datetime(df_info_price['运行日期']) <= pd.to_datetime(end_date))]
            ...
            with tab1:
                ...
                c21, c22 = st.columns(2)
                if mode == '开始日期-结束日期':
                    begin_date = c21.selectbox('开始日期',df_info_compete['运行日期'].sort_values(ascending=False).unique().tolist(),index=0)
                    end_date = c22.selectbox('结束日期', df_info_compete['运行日期'][pd.to_datetime(df_info_compete['运行日期']) >= pd.to_datetime(begin_date)].sort_values(ascending=False).unique().tolist(), index=0)
                    df_compete = df_info_compete[(pd.to_datetime(df_info_compete['运行日期']) >= pd.to_datetime(begin_date)) & (pd.to_datetime(df_info_compete['运行日期']) <= pd.to_datetime(end_date))]
                    datelist = pd.date_range(begin_date, end_date, freq='D').strftime("%Y-%m-%d").to_list()
                elif mode == '日期':
                    datelist = st.multiselect('请选择日期' ,df_info_compete['运行日期'].sort_values(ascending=False).unique().astype(str).tolist(),df_info_compete['运行日期'].sort_values(ascending=False).unique().astype(str).tolist()[0])
                ...
                # st_pyecharts(price_jjkj, theme=ThemeType.CHALK)
                # page = Page()
                # page.add(energy, market_price, price_jjkj)
                # page.render('折线图.html')
                # scdt = st.tabs(['市场动态'])
                # scdt = st.sidebar.button('市场动态')

                # if scdt:
                # st.date_input('开始时间', datetime.datetime.now())
                # components.html(energy_html, width=900, height=800)
                # components.html(market_price_html, width=900, height=800)
                # st_pyecharts(energy, theme=ThemeType.LIGHT)
                # st_pyecharts(market_price, theme=ThemeType.LIGHT)
                # grid = (
                #     Grid()
                #     .add(energy, grid_opts = opts.GridOpts(pos_bottom='60%'))
                #     .add(market_price, grid_opts=opts.GridOpts(pos_top="60%"))
                # )
                # tab = (
                #     Tab()
                #     .add(grid, '市场动态')
                # )
                # tab.render('市场动态.html')
                # st.date_input(
                #     'When', datetime.date(2019, 7, 6))
                # st_pyecharts(energy, theme=ThemeType.LIGHT)
                # st_pyecharts(market_price, theme=ThemeType.LIGHT)
                #
                #
